...
//        minAttempts = this.findMinAttemptsTaken();
    ...
    private int findHighestPointsEarned() {
        for (int i=0; i<scores.size(); i++) {
            if (scores.get(i).getPointsEarned() > highPointsEarned) {
                highPointsEarned = scores.get(i).getPointsEarned();
            }
        }
    ...
    private int findMinAttemptsTaken() {
        for (int i=0; i<scores.size(); i++) {
            if (scores.get(i).getNumAttempts() < minAttempts) {
                minAttempts = scores.get(i).getNumAttempts();
            }
        }
        return minAttempts;
    }
